...
export default function AuditLogTimeline({
  ...
  return (
    <div
      className="p-6 rounded-3xl border text-right space-y-4 shadow-md relative overflow-hidden transition-all duration-300 hover:shadow-[0_0_20px_rgba(16,185,129,0.06)] font-sans"
      style={{
        background: `radial-gradient(circle at top right, rgba(16, 185, 129, 0.04) 0%, ${theme.card} 100%)`,
        borderColor: theme.border
      }}
    >
      <div className="absolute left-0 top-0 w-48 h-48 bg-emerald-500/5 rounded-full blur-3xl pointer-events-none" />
      
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pb-3 relative z-10">
        ...
        <div>
          <h3 className="text-sm font-black flex items-center justify-end gap-1.5 text-white">
      ...
      <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
        {filteredLogs.map((log) => (
          <div
            key={log.id}
            className="p-3.5 rounded-2xl border flex items-start justify-between gap-3 text-right transition-all duration-300 hover:scale-[1.01] hover:shadow-lg relative overflow-hidden"
            style={{
              background: `radial-gradient(circle at top right, rgba(16, 185, 129, 0.02) 0%, rgba(9, 15, 29, 0.95) 100%)`,
              borderColor: theme.border
            }}
          >
            <div className="flex flex-col items-start text-left shrink-0 gap-1 mt-0.5 relative z-10">
            ...
            <div className="flex gap-2.5 items-start text-right grow relative z-10">
